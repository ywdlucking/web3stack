...
async function main() {
  ...
  const provider = await new hre.ethers.getDefaultProvider();  //连接网络
  const card = await new hre.ethers.Contract(contractAddress,abi,provider);
  // const card = await hre.ethers.Contract('0x5FbDB2315678afecb367f032d93F642f64180aa3');
  // await card.waitForDeployment();
  const tx = await card.mint();
  console.log(
    `Card mint to ${tx}`
  );
}
